...
namespace BetBot
{
    public partial class ManualPokerPlayer : Form
    {
        public ManualPokerPlayer()
        {
        ...
        private void resultBtn_Click(object sender, EventArgs e)
        {
            ...
            if (holeCards.Count == 2)
            {
                ...
                else
                {
                    ...
                    int betAmount = BotOperations.WhatToBet(odds, int.Parse(tableChipsTxt.Text) + int.Parse(currentlyBettingTxt.Text),int.Parse(opponentBetTxt.Text), int.Parse(myChipsTxt.Text), bankRoll, out double expectedValue) + int.Parse(currentlyBettingTxt.Text);
                    if (expectedValue <= 0)
                    {
                        MessageBox.Show("fold!");
                    }
                    else if (betAmount - int.Parse(currentlyBettingTxt.Text) == int.Parse(opponentBetTxt.Text))
                    {
    ...
    public static class BotOperations
    {
        ...
        private static double getRiskOfRuin(double odds, int i, int bankRoll)
        {
            ...
            {
                return Math.Pow((1 - (odds - (1 - odds))) /(1 + (odds - (1 - odds))), units);
            }
            Debug.WriteLine("exponent:" + bankRoll / i);

            return i == 0 ? 0 : riskOfRuin(bankRoll / i);
            //mindre exponent = högre ror
        ...
        public static int WhatToBet(double odds,int tableChips, int chipOffer, int myChips, int bankRoll, out double expectedValue)
        {
            int bet = 0;
            double maxEv = 0;
            int limit = (tableChips + chipOffer) < 500 ? chipOffer + 500 : myChips;
            for (int i = chipOffer; i <= limit; i++)
            {
                if ((i == chipOffer) != (i >= chipOffer * 2)){
                    double currentEV = (odds * (tableChips + i)) + ((1 - odds) * -(i));
                    double ror = getRiskOfRuin(odds, i, bankRoll);
                    Debug.WriteLine($"EV at betting {i} is {currentEV}, risk of ruin is {ror}");
        ...
        private static (double heroPoints, double villainPoints) updateScore(ulong holes, ulong villainHoles, ulong currentHand)
        {
            ...
            //Debug.WriteLine($"herohand:{HandEvaluation.ParseAsCardString(heroDummy)} score:{heroHand.mainScore},{heroHand.tieBreaker}  villainhand:{HandEvaluation.ParseAsCardString(villainDummy)} score:{villainHand.mainScore},{villainHand.tieBreaker}");
            ...
            {
                case 1:
                    //Debug.WriteLine($"win: hero: {HandEvaluation.ParseAsCardString(heroDummy)} villain:{HandEvaluation.ParseAsCardString(villainDummy)}");
                    return (1, 0);
                case -1:
                    //Debug.WriteLine($"loss: hero: {HandEvaluation.ParseAsCardString(heroDummy)} villain:{HandEvaluation.ParseAsCardString(villainDummy)}");
                    return (0, 1);
                default:
                    //Debug.WriteLine($"tie: hero: {HandEvaluation.ParseAsCardString(heroDummy)} villain:{HandEvaluation.ParseAsCardString(villainDummy)}");
                    return (0.5, 0.5);
            }
        ...
        public static double GetOdds(List<KeyValuePair<int, int>> holeCardsKvpList, List<KeyValuePair<int, int>> tableKvpList)
        {
            ...
            {
                holesCount++;
                Debug.WriteLine(holesCount);
                ulong currentAvailableCards = availableCards & ~villainHoles;
                if (tableKvpList.Count < 5)
                {
                    Parallel.ForEach(HandEvaluation.CardCombos(currentAvailableCards.ToIEnum(), 5 - tableKvpList.Count), cardAdditions =>//all combinations of cards that may be added to the existing table
                    {
            ...
            Debug.WriteLine(heroWins / (villainWins + heroWins));
        
            //return heroWins / (heroWins + villainWins);
            return heroWins / (villainWins + heroWins);
    ...
    public static class HandEvaluation
    {
        ...
        public static string ParseAsCardString(ulong bf)
        {
            ...
            for (int i = 17; i <= 29; i++)
            {
                if ((bf & (1UL << i)) > 0)
                {
                    kvpList.Add(new KeyValuePair<int, int>(i - 15, 1));
                }
            ...
            for (int i = 32; i <= 44; i++)
            {
                if ((bf & (1UL << i)) > 0)
                {
                    kvpList.Add(new KeyValuePair<int, int>(i - 30, 2));
                }
            ...
            for (int i = 47; i <= 60; i++)
            {
                if ((bf & (1UL << i)) > 0)
                {
                    kvpList.Add(new KeyValuePair<int, int>(i - 45, 3));
                }
            }
            return Parser.ToCardString(kvpList);
        ...
        struct Card : IComparable<Card>
        {
            ...
            {
                return Instances.CompareTo(card.Instances) != 0 ? Instances.CompareTo(card.Instances) : Rank.CompareTo(card.Rank);
            }
        }
        ...
        private static long getTieBreaker(ulong ranksField)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                if (!instances.ContainsKey(highestRank))
                {
                    instances.Add(highestRank, 1);
                }
                else
                {
                    instances[highestRank] = instances[highestRank] + 1;
                }
                ranksField &= ~(1UL << pos);
            }
            foreach (var kvp in instances)
            {
                ...
                {
                    hand.Add(new Card { Rank = kvp.Key, Instances = kvp.Value });
                }
            }
            hand.Sort();
            long result = hand[4].Rank << 16 | hand[3].Rank << 12 | hand[2].Rank << 8 | hand[1].Rank << 4 | hand[0].Rank << 0;
            lock (lockObj)
            {
                _tieBreakerTable[ranksField] = result;
        ...
        public static (int mainScore, long tieBreaker) GetFullScore(ulong bf)
        {
            ...
            (int mainScore, long tieBreaker) result = (mainScore, solo == 16444 ? -1 : getTieBreaker(ranksField));
            lock (lockObj)
            {
        ...
        public static (int mainScore, long tieBreaker) HandToPlay(ulong holes, ulong cardsOnTable)
        {
            ...
            (int mainScore, long tieBreaker) tableScore = GetFullScore(cardsOnTable);
            if ((tableScore.mainScore.CompareTo(max.mainScore) != 0 ? tableScore.mainScore.CompareTo(max.mainScore) : tableScore.tieBreaker.CompareTo(max.tieBreaker)) > 0)
            {
                max = tableScore;
            }
